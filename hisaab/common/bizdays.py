...
**Why this lives in ``common/`` rather than under the generator.** It moved here in
Phase 3, for the same reason ``money.py`` did in Phase 2: both halves of the system
need it and neither may import the other. The generator computes ``settled_on`` over
this calendar; Phase 3's ``matcher/blocking.py`` counts the same business days to
decide whether a settlement is inside a credit's date window. The matching path may
not import ``hisaab.generator`` -- it knows the fee rates, the T+n cycle and the
narration templates, and ``tools/check_isolation.py`` check 6 enforces that -- so a
calendar sitting there would have forced the matcher to duplicate it.

Duplication is the right guard for a *schema* (see the header tuples in
``matcher/load.py``, and ``SUPPORTED_SCHEMA_VERSION`` in ``scoring/truth_io.py``),
because drift then surfaces as a loud mismatch. It is the wrong guard for *logic*:
two calendars that disagree by a day produce a plausible wrong answer and nothing
detects it. One implementation, two callers.
